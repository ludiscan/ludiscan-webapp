...
// Import pre-generated docs data for production use
// These files are generated by scripts/generate-docs-json.js during build
import generatedDocsGroups from '@generated/docs-groups.json';
import generatedDocsPages from '@generated/docs-pages.json';

// Type for the generated pages map
type GeneratedPagesMap = Record<string, { slug: string; frontmatter: DocPage['frontmatter']; content: string }>;
 ...
 * Falls back to pre-generated JSON if filesystem is not available (serverless)
 */
export async function getDocBySlug(slug: string): Promise<DocPage | null> {
  ...
  // Fallback to pre-generated pages if filesystem is not available
  const generatedPages = generatedDocsPages as GeneratedPagesMap;
  const generatedPage = generatedPages[slug];
  if (generatedPage) {
    return {
      slug: generatedPage.slug,
      frontmatter: generatedPage.frontmatter,
...
export async function getDocSlugs(): Promise<string[]> {
  ...
  // If we have pages from filesystem, use them
  ...
  // Fallback to pre-generated pages if filesystem is not available
  const generatedPages = generatedDocsPages as GeneratedPagesMap;
  return Object.keys(generatedPages);
}
