...
// Import pre-generated docs for production use
 ...
 * In production/serverless environment, uses pre-generated JSON
 * In development, loads from disk
 */
export async function getDocBySlug(slug: string): Promise<DocPage | null> {
  // First, try to get from pre-generated JSON (for serverless/production)
  const preGeneratedPage = (generatedDocsPages as Record<string, DocPage>)[slug];
  if (preGeneratedPage) {
    return preGeneratedPage;
  }

  // If not found in pre-generated JSON, try loading from disk (for development)
