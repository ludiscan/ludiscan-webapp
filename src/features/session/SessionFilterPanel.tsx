    ...
    <div className={className}>
      <FlexRow className={`${className}__header`} align='center'>
        <FlexRow gap={8} align='center'>
          ...
          {hasActiveFilters && <span className={`${className}__badge`}>{Object.values(filters).filter((v) => v !== undefined && v !== '').length}</span>}
        </FlexRow>
        <FlexRow gap={8}>
          {hasActiveFilters && (
      ...
      {isExpanded && (
        <FlexColumn className={`${className}__content`} gap={16}>
          ...
          <button type='button' className={`${className}__advancedToggle`} onClick={() => setShowAdvanced(!showAdvanced)}>
            <Text
          ...
          {showAdvanced && (
            <FlexColumn gap={16}>
              ...
              <div className={`${className}__filterItem ${className}__filterItem--full`}>
                ...
                />
                <span className={`${className}__hint`}>例: platform:Android, is:playing, is:finished, app_version:1.0.0</span>
              </div>
            </FlexColumn>
          )}
        </FlexColumn>
